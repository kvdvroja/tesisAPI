...
{
    [ApiController]
    [Route("api/v1/Asesor/{proceso}/{modo}")]
    [Authorize]
    public class AsesorController : ControllerBase
    {
        ...
        public AsesorController(TesisService oracle)
        {
            _oracle = oracle;
        }
        ...
        {
            { "CUR", "TESIMGR.KTS_GENERAL.KTS_GENE_TSRPERS_CUR" },
        ...
        public async Task<IActionResult> Ejecutar(string proceso, string modo, [FromQuery] string? codigo, [FromBody] object? payload)
        {
            ...
            if (!Utilidades.Acciones.AsesorModosValidos.Contains(modo))
