...
namespace tesisAPI.Services
{
    public class TareaService
    {
        ...
        public TareaService(OConnection factory)
        {
            _factory = factory;
        }

        public async Task<string> EjecutarAUD(string nombreProc, string modo, object payload)
        {
            ...
            cmd.Parameters.Add("P_DATA", OracleDbType.Clob).Value = dataJson;
            cmd.Parameters.Add("P_MSG", OracleDbType.Varchar2, 4000).Direction = ParameterDirection.Output;

            await cmd.ExecuteNonQueryAsync();

            return cmd.Parameters["P_MSG"].Value?.ToString() ?? "Sin mensaje.";
        }
